...
function initializePopup() {
  ...
  setTimeout(() => {
    addLog("TK SmartFlow v2.4.2 hazır - Countdown timer iyileştirmeleri ile");
      ...
      chrome.tabs.sendMessage(
        ...
        function (response) {
          ...
          setTimeout(() => {
            ...
            if (chrome.runtime.lastError) {
              addLog(
                `❌ Mesaj gönderilemedi: ${chrome.runtime.lastError.message}`,
                "error"
              );
              addLog(
                "💡 Content script henüz hazır değil - THY sayfasını yenileyin",
                "warning"
              );
              ...
              addLog(
                `⚠️ Çalıştırılamadı: ${response.message || "Bilinmeyen sebep"}`,
                "warning"
              );
            } else {
        ...
        function (response) {
          ...
          setTimeout(() => {
            ...
            if (chrome.runtime.lastError) {
              ...
              addLog(
                "💡 Content script henüz yüklenmemiş olabilir - Sayfayı yenileyin",
                "warning"
              );
              ...
              if (data.tableRows > 0 || data.dashboardCards > 0) {
                addLog(
                  `📊 DOM: ${data.tableRows} tablo satırı, ${data.dashboardCards} dashboard kartı`,
                  "info"
                );
                if (data.prRows !== undefined && data.prRows > 0) {
                  addLog(
                    `🎯 PR Bulundu: ${data.prRows} adet PR tespit edildi`,
                    "success"
                  );
              ...
              addLog(
                `⚙️ İşlem Durumu: ${statusIcon} ${statusText}`,
              ...
              const intervalText = data.autoRunInterval
                ...
                : "DURMUŞ";
              addLog(
                `⏱️ Interval: ${intervalIcon} ${intervalText}`,
              ...
              if (data.processingLock !== undefined) {
                ...
                addLog(
                  `🔐 PR İşlem Kilidi: ${lockIcon} ${lockText}`,
              ...
              addLog(
                `❌ Analiz hatası: ${response.message || "Bilinmeyen hata"}`,
                "error"
              );
            } else {
              addLog("⚠️ Content script yanıt vermedi", "warning");
...
function loadSettings() {
  chrome.storage.local.get(["waitTimeout"], (result) => {
    ...
    updateWaitTimeoutInfo(); // Dakika bilgisini güncelle
  });
}
...
function handleWaitTimeoutChange(event) {
  ...
  if (isNaN(value) || event.target.value.trim() === "") {
    ...
    updateWaitTimeoutInfo(); // Span'ı güncelle
    addLog(
      `⚠️ Geçersiz değer! Varsayılan yenileme aralığı ${CONFIG.WAIT_TIMEOUT.DEFAULT} saniye ayarlandı`,
      "warning"
    );
  ...
  if (value < CONFIG.WAIT_TIMEOUT.MIN) {
    ...
    updateWaitTimeoutInfo(); // Span'ı güncelle
    addLog(
      `⚠️ Minimum yenileme aralığı ${CONFIG.WAIT_TIMEOUT.MIN} saniye (${minMinutes} dakika)`,
      "warning"
    );
  ...
  if (value > CONFIG.WAIT_TIMEOUT.MAX) {
    ...
    updateWaitTimeoutInfo(); // Span'ı güncelle
    addLog(
      `⚠️ Maksimum yenileme aralığı ${CONFIG.WAIT_TIMEOUT.MAX} saniye (${maxMinutes} dakika)`,
      "warning"
    );
  ...
  chrome.storage.local.set({ waitTimeout: value }, () => {
    addLog(
      `✅ Sayfa yenileme aralığı ${value} saniye olarak ayarlandı`,
      "success"
    );
    console.log("💾 Yenileme aralığı kaydedildi:", value);
  });
}
